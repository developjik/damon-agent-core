...
async fn main() -> anyhow::Result<()> {
    ...
    // Same exposure for the channel tokens — argv is world-readable.
    if matches.value_source("app_token") == Some(clap::parser::ValueSource::CommandLine) {
        eprintln!("warning: --app-token is visible in process lists; prefer SLACK_APP_TOKEN env");
    }
    if matches.value_source("bot_token") == Some(clap::parser::ValueSource::CommandLine) {
        eprintln!("warning: --bot-token is visible in process lists; prefer SLACK_BOT_TOKEN env");
    }
